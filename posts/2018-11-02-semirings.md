...
Moreover I focused on the cardinality of only the types we need, but for completions sake, we could also add instances of `Cardinality` for things like `A => B` , `Option[A]` or `Ior[A, B]`.
These are:
