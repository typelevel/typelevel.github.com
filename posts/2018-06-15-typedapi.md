...
Again, we get some `Segment` with a name `K` and a type `V` and an intermediate aggregation state we will update by adding a placeholder to `El`, the name to `KIn` and the value type to `VIn`.
...
We folded our api type into the new representation. Now we use that to derive a function which collects all the data necessary to make a request.
...
I already hear the "your function signature is so big ..." jokes incoming, but this is basically what we will (and want to) end up with when doing type-level programming. In the end, our types have to express the logic of our programm and that needs some space.
