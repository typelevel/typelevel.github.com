...
Consider the following somewhat-realistic NIO API:
...
public interface Channel {
  public void sendBytes(byte[] chunk, Response<Void> handler);
  public void receiveBytes(Response<byte[]> handler);
}
```
...
def send(c: Channel, chunk: Array[Byte]): IO[Unit] = {
  IO async { cb =>
    c.sendBytes(chunk, new Response[Void] {
      def onError(t: Throwable) = cb(Left(t))
      def onSuccess(v: Void) = cb(Right(()))
    })
  }
}
...
As mentioned earlier (about 10000 words ago…), the cats-effect project not only provides a concrete `IO` type with a lot of nice features, it also provides a set of abstractions characterized by typeclasses and associated laws.  These abstractions collectively define what it means to be a type which encapsulates side-effects in a pure fashion, and they are implemented by `IO` as well as several other types (including `fs2.Task` and `monix.Task`).  The hierarchy looks like this:
