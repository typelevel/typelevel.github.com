...
EitherT takes any monad, like our Writer, and adds the effects of Either. It is constructed with three types: `EitherT[M. A, B]` where M is the monad, A is the failure type and B is the success type. In Treelog, M is a Writer, A is a String and B is the type of the result.

When used in a for-comprehension, the values of the computation are available to work with whilst recording success and failure, a writing the log.
