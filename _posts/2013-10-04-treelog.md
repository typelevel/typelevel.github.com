...
case class DescribedLogTreeLabel(description: String, success: Boolean, annotations: Set[Annotation]) extends LogTreeLabel

case class UndescribedLogTreeLabel[A](success: Boolean, annotations: Set[Annotation] = Set[A]()) extends LogTreeLabel[Annotation] { … }
```

Note that originally we simply used an `Option[String]` rather than this implementation, but because extra information had to be carried around we came up with LogTreeLabel.
...
In Treelog we distinguish between tree nodes that describe a computation, a `DescribedLogTreeLabel`, and an `UndescribedLogTreeLabel` which is a Tree with no description in the root. In the example above, the root node is a DescribedLogTreeLabel containing `Calculating sum`. This is an important distinction that informs the way trees must be combined by our Treelog Monoid, which the Writer needs.
...
Note that we always end up with an Undescribed Tree since there is no meaningful way to combine descriptions of child nodes. In the example above the tree contains two leaves: "Got a 3" and "Got a 5". Concatenating those descriptions won't mean much. A meaningful description would be "Summing a and b", which we could have done like this:
```scala
val r: Writer[String, Int] = 
  "Summing a and b" ~< for {
    a ← 3.set("Got a 3.")
...
The [quadratic roots](https://github.com/lancewalton/treelog/blob/master/src/test/scala/QuadraticRootsExample.scala) example is a good one to see this.
...
```
To retrieve the underying monad, the Writer, back from EitherT we call `run` which returns the Writer containing scalaz's Either type `\/` (which is more useful than scala's built-in Either).
