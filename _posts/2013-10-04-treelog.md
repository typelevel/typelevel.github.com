...
The tree will have a root node with `"Calculating sum"` and children describing the values of x and y and their sum. The value is Scalaz's Either type, `\/`, which will contain the result of the computation in `\/-`, the right side, or a failure message in `-\/`, the left side.

In Treelog we distinguish between tree nodes that describe a computation with a description as in the example above, a so-called `DescribedLogTreeLabel', and an `UndescribedLogTreeLabel` which is a Tree with no description in the root.
...
Syntactic Sugar
---------------
Treelog makes use of some syntactic sugar inspired by Tony Morris's blog on Writer. In the example above, `~>` is a method on an implicitlt constructed class in Treelog which takes a value, x: T, and returns a `DescribedComputation[T]` which expands out to an EitherT whose value is `\/-(x)`, and writtem value is a leaf node containing the description `foo's value is [whatever x's value was]`.
...
I am going to be ritually sacrificed to the Gods of FP for saying this, but I find it helps people coming from an OO background: think of a Monad Transformer as a decorator. It encapsulates a monad, provides extra behaviour, and through map and flatMap, or for-comprehensions, allows you to work with the monad as if the transformer wasn't there.
...
```
You can see that to retrieve the underying monad, the Writer, back from EitherT we call `run` which returns our writer with scalaz's Either type `\/` which is more useful than scala's bulit-in Either.
