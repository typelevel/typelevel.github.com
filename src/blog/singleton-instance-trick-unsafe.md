...
[appears in variant or phantom position in the actual data involved](liskov-lifting.md).
...
[the `Liskov`-lifting previously discussed](liskov-lifting.md):
...
In addition, **`A` is invariant**, so after going up to
...
the end of [“When can Liskov be lifted?”](liskov-lifting.md),
